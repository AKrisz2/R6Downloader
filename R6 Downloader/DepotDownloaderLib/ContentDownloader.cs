    ...
    static class ContentDownloader
    {
            ...
            else
            {
                ...
                if (neededChunks.Count() == 0)
                {
                    lock (depotDownloadCounter)
                    {
                        ...
                        HomePage.dispatcherQueue.TryEnqueue(async () =>
                        {
                            HomePage._downloadText.Text += $"\n{((depotDownloadCounter.SizeDownloaded / (float)depotDownloadCounter.CompleteDownloadSize) * 100.0f):F2}% {fileFinalPath}";
                            HomePage._downloadScroller.ScrollToVerticalOffset(HomePage._downloadScroller.ExtentHeight);
                        });
            ...
            ProtoManifest.ChunkData chunk)
        {
            ...
            if (remainingChunks == 0)
            {
                ...
                HomePage.dispatcherQueue.TryEnqueue(async () =>
                {
                    HomePage._downloadText.Text += $"\n{((sizeDownloaded / (float)depotDownloadCounter.CompleteDownloadSize) * 100.0f):F2}% {fileFinalPath}";
                    HomePage._downloadScroller.ScrollToVerticalOffset(HomePage._downloadScroller.ExtentHeight);
                });
                if (DepotDownloaderLib.onConsoleOutput != null)
