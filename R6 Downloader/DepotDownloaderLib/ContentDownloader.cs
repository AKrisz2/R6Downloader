...
using SteamKit2;
using SteamKit2.CDN;
using static SteamKit2.Steam3Manifest.FileMapping;
...
namespace DepotDownloader
{
    public static class DownloadNotifier
    {
        ...
        public static void RaiseDownloadCompleted()
        {
    ...
    static class ContentDownloader
    {
        ...
        public static ulong TotalUncompressedToDownload = 0;
        public static ulong TotalSizeDownloaded = 0;
        public static ulong TotalChunkSizeDownloaded = 0;
        public static float PercentDownloaded = 0;
        public static int DownloadedDepotCount = 0;
        public static int DepotsToDownload = 0;
        ...
        private static async Task DownloadSteam3Async(List<DepotDownloadInfo> depots)
        {
            ...
            Console.WriteLine($"\nTotal download size: {ConvertBytes(TotalUncompressedToDownload)}");
            Console.WriteLine("Total downloaded: {0} from {1} depots",
                ConvertBytes(downloadCounter.TotalBytesUncompressed), depots.Count);
            DownloadNotifier.RaiseDownloadCompleted();
        }
        ...
        private static async Task<DepotFilesData> ProcessDepotManifestAndFiles(CancellationTokenSource cts, DepotDownloadInfo depot)
        {
            ...
            TotalUncompressedToDownload += depotCounter.CompleteDownloadSize;
            HomePage.dispatcherQueue.TryEnqueue(async () =>
            {
                HomePage._progressText.Text = $"0.00GB/{ConvertBytes(TotalUncompressedToDownload)}";
            });
            ...
            else
            {
                ...
                if (neededChunks.Count == 0)
                {
                    lock (depotDownloadCounter)
                    {
                        ...
                        TotalSizeDownloaded += file.TotalSize;
                        if (DownloadedDepotCount == 0)
                        {
                            PercentDownloaded = (TotalSizeDownloaded / (float)TotalUncompressedToDownload) * 100.0f;
                        }
                        else
                        {
                            PercentDownloaded = (TotalSizeDownloaded / (float)TotalUncompressedToDownload) * 100.0f;
                        }
                        Console.WriteLine("{0,6:#00.00}% {1}", PercentDownloaded, fileFinalPath);
                        HomePage.dispatcherQueue.TryEnqueue(async () =>
                        {
                            HomePage._progressText.Text = $"{ConvertBytes(TotalSizeDownloaded)}/{ConvertBytes(TotalUncompressedToDownload)}";
                            HomePage._downloadProgressBar.Value = PercentDownloaded;
                            HomePage._downloadText.Text += $"\n{PercentDownloaded:F2}% {fileFinalPath}";
                            HomePage._downloadScroller.ScrollToVerticalOffset(HomePage._downloadScroller.ExtentHeight);
                        });
                    }
        ...
        {
            TotalChunkSizeDownloaded = TotalSizeDownloaded;
            cts.Token.ThrowIfCancellationRequested();
            ...
            lock (depotDownloadCounter)
            {
                ...
                TotalChunkSizeDownloaded += chunk.UncompressedLength;

                //Console.WriteLine($"{ConvertBytes(TotalChunkSizeDownloaded)} downloaded");

                if (DownloadedDepotCount == 0)
                {
                    PercentDownloaded = (TotalChunkSizeDownloaded / (float)TotalUncompressedToDownload) * 100.0f;
                }
                else
                {
                    PercentDownloaded = (TotalChunkSizeDownloaded / (float)TotalUncompressedToDownload) * 100.0f;
                ...
                HomePage.dispatcherQueue.TryEnqueue(async () =>
                {
                    HomePage._progressText.Text = $"{ConvertBytes(TotalChunkSizeDownloaded)}/{ConvertBytes(TotalUncompressedToDownload)}";
                    HomePage._downloadProgressBar.Value = PercentDownloaded;
                });
            }
